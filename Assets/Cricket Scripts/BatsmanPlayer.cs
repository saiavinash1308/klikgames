...
public class BatsmanPlayer : MonoBehaviour
{
    ...
    public void ControlMovement()
    {
        ...
        if (adjustedValue != 0)
        {
            float predictedX = transform.position.x + (adjustedValue * movespeed * Time.deltaTime);

            // Serialize data for socket emission
            /*   var data = new
               {
            ...
            // Uncomment the following line if socket emission is needed
            // socketmanager.EmitEvent("MOVE_BATSMAN", jsonData);

            // Move the batsman
            MoveBatsman2(predictedX);
           // MoveBatsManFromSocket(adjustedValue, predictedX);
        }
    }
    ...
    public void OnMoveLeftPressed()
    {
        ...
        batstate = BatState.moving;
        moveLeft = true;
        moveRight = false; // Disable the opposite direction
        anim.Play("Left");
    }
    ...
    public void OnButtonReleased()
    {
        ...
        moveLeft = false;
        moveRight = false;
        MoveBatsman2(transform.position.x);
        anim.Play("Idle");
    }

    public void MoveBatsman2(float targetXval)
    {
        float clampedX = Mathf.Clamp(targetXval, 22f, 25f);
        Vector3 targetPos = new Vector3(clampedX, transform.position.y, transform.position.z);
        // Apply the movement
        transform.position = Vector3.MoveTowards(transform.position, targetPos, movespeed * Time.deltaTime);
    }

    public void MoveBatsManFromSocket(float adjustedValue, float predictedX)
    {
...
public void PressedSlider()
    ...
    {
        Debug.Log("Hitting bat from socket");
        batstate = BatState.hitting;
        anim.Play("Hit");
        ResetIdle();
    }
