...
public class PathPointFor2Player : MonoBehaviour
{
    PathPointFor2Player[] pathpointtomoveon_;
    public PathObjectParentFor2Player pathObjectParent;
    public List<PlayerPiecesFor2Player> PlayerPiecesList = new List<PlayerPiecesFor2Player>();

    [SerializeField] private GameObject killingPopPrefab; // Add this line

    void Start()
    {
        pathObjectParent = GetComponentInParent<PathObjectParentFor2Player>();
    }

    public bool AddPlayerPieces(PlayerPiecesFor2Player playerPiece_)
    {
        ...
        if (this.name == "CenterPath")
        {
            ...
            GameManagerFor2Player.game.PlayCenterPathAudio();

            if (GameManagerFor2Player.game.HasPlayerWon(playerColor))
            {
                GameManagerFor2Player.game.shouldRollAgain = false;
                GameManagerFor2Player.game.transferDice = true;
                GameManagerFor2Player.game.manageRolingDice[GetPlayerIndex(playerColor)].gameObject.SetActive(false);
    ...
    private void AdjustSortingOrder(PlayerPiecesFor2Player playerPiece_)
    {
        foreach (var piece in PlayerPiecesList)
        {
            piece.GetComponentInChildren<SpriteRenderer>().sortingOrder = 0; // Reset sorting order
    ...
    IEnumerator RevertonStart(PlayerPiecesFor2Player playerPiece_)
    {
        //if (playerPiece_.name.Contains("Blue"))
        //{
        ...
        {
            GameManagerFor2Player.game.redOutPlayers -= 1;
            pathpointtomoveon_ = pathObjectParent.RedPlayerPathPoint;
        }
        //else if (playerPiece_.name.Contains("Green"))
        //{
        ...
        {
            GameManagerFor2Player.game.yellowOutPlayers -= 1;
            pathpointtomoveon_ = pathObjectParent.YellowPlayerPathPoint;
        }
    ...
    IEnumerator AddKillerPoints(PlayerPiecesFor2Player killerPiece_)
    {
        //if (killerPiece_.name.Contains("Blue"))
        //{
        //    GameManager.game.bluepoints += 50;
        //}
        if (killerPiece_.name.Contains("Red"))
        {
            GameManagerFor2Player.game.redpoints += 50;
        }
        //else if (killerPiece_.name.Contains("Green"))
        //{
        //    GameManager.game.greenpoints += 50;
        ...
        {
            GameManagerFor2Player.game.yellowpoints += 50;
        }

        yield return null;
    }

    void ResetToBasePoint(PlayerPiecesFor2Player playerPiece_)
    {
        Debug.Log(playerPiece_.name);
    ...
    void AddPlayer(PlayerPiecesFor2Player playerPiece_)
    {
        PlayerPiecesList.Add(playerPiece_);
        RescaleandRepositiononAllPlayerPiece();
    }

    public void RemovePlayerPieces(PlayerPiecesFor2Player playerPiece_)
    {
        if (PlayerPiecesList.Contains(playerPiece_))
        {
            PlayerPiecesList.Remove(playerPiece_);
    ...
    void RescaleandRepositiononAllPlayerPiece()
    {
        ...
        float baseSpacing = 20.0f; // Base spacing between pieces
