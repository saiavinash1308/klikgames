...
public class RollingDiceFor2Player : MonoBehaviour
{
    [SerializeField] Sprite[] numberSprites;
    [SerializeField] SpriteRenderer numberSpriteHolder;
    [SerializeField] SpriteRenderer rolingdiceanimation;
    ...
    public void OnMouseDown()
{
    if (GameManagerFor2Player.game.canDiceRoll && GameManagerFor2Player.game.rolingDice == this)
    {
            //GameManagerFor2Player.game.ResetCountdown();
        generateRandomNumberonDice = StartCoroutine(RollingDiceCoroutine());
            //GameManagerFor2Player.game.StartCountdown();
        }
}
    ...
    IEnumerator RollingDiceCoroutine()
    {
        ...
        yield return new WaitForEndOfFrame();
        if (GameManagerFor2Player.game.canDiceRoll)
        {
            GameManagerFor2Player.game.canDiceRoll = false;
            diceSound.PlaySound();
            numberSpriteHolder.gameObject.SetActive(false);
            rolingdiceanimation.gameObject.SetActive(true);
            yield return new WaitForSeconds(0.8f);
            ...
            numberSpriteHolder.sprite = numberSprites[numberGot];
            numberGot += 1;
            GameManagerFor2Player.game.UpdatePlayerPoints(numberGot);

            GameManagerFor2Player.game.numberofstepstoMove = numberGot;
            GameManagerFor2Player.game.rolingDice = this;
            numberSpriteHolder.gameObject.SetActive(true);
            rolingdiceanimation.gameObject.SetActive(false);
            yield return new WaitForEndOfFrame();
    ...
    bool CanPlayerMove()
    {
        GameManagerFor2Player gm = GameManagerFor2Player.game;
        List<PlayerPiecesFor2Player> pieces = gm.GetPlayerPiecesForCurrentDice();

        foreach (PlayerPiecesFor2Player piece in pieces)
        {
            if (piece.CanMove(gm.numberofstepstoMove))
            {
                return true;
